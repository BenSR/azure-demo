...
variable "runner_management_pat" {
  type        = string
  sensitive   = true
  description = "GitHub PAT with manage_runners:repo scope. Passed to the Custom Script Extension (encrypted in protected_settings) so setup-runner.sh can exchange it for a fresh 1-hour registration token at VM provisioning time. Pass via RUNNER_MANAGEMENT_PAT GitHub secret — never store in tfvars."
}

variable "deploy_storage_account_name" {
  type        = string
  description = "Name of the state/deploy storage account (created by prepare-azure-env.sh). Used to locate setup-runner.sh for the Custom Script Extension. Pass via TF_STATE_STORAGE_ACCOUNT — the same secret already used for the Terraform backend."
}
